bersih
bi = '\ 033 [34; 1m' #biru
i = '\ 033 [32; 1m' #ijo
pur = '\ 033 [35; 1m' #purple
cy = '\ 033 [36; 1m' #cyan
saya = '\ 033 [31; 1m' #merah
pu = '\ 033 [37; 1m' #putih
ku = '\ 033 [33; 1m' #kuning
gema
gema
python2 meizu.py
gema
bersih
gema
echo $ red "Alat akan di instal dalam waktu 5 detik"
echo $ red "harap tunggu, tunggu yaa sayang: v maklum si mamad jomblo :("   
tidur 1
gema '' '\ a
\ 033 [34; 1j _
\ 033 [34; 1m / |
\ 033 [34; 1m | |
\ 033 [34; 1m | |
\ 033 [34; 1m _ | _ | _ '' '
tidur 1
pembaruan apt && peningkatan apt
untuk menginstal nano
untuk menginstal git
pkg instal python python2 vim figlet curl
bersih
gema
gema
gema
gema
echo $ red "Alat akan di instal dalam waktu 5 detik"
echo $ red "harap tunggu menunggu yaa sayang: v"
tidur 1
gema '' '\ a
\ 033 [32; 1m ____
\ 033 [32; 1m | ___ \
\ 033 [32; 1m __) |
\ 033 [32; 1m / __ /
\ 033 [32; 1m | _____ | ''
tidur 1
untuk memasang php
Instal pip2 mekanis
pip2 instal lolcat
permintaan instalasi pip2
bersih
gema
gema
gema
gema
echo $ red "Alat akan di instal dalam waktu 5 detik"
echo $ red "harap tunggu menunggu yaa sayang: v"
tidur 1
gema '' '\ a
\ 033 [35; 1m _____
\ 033 [35; 1m | ___ /
\ 033 [35; 1m | _ \
\ 033 [35; 1m ___) |
\ 033 [35; 1m | ____ / '' '
tidur 1
bersih
gema
gema
gema
gema
echo $ red "Alat akan di instal dalam waktu 5 detik"
echo $ red "harap tunggu menunggu yaa sayang: v"
tidur 1
gema '' '\ a
\ 033 [33; 1m _ _
\ 033 [33; 1m | || |
\ 033 [33; 1m | || | _
\ 033 [33; 1m | __ _ |
\ 033 [33; 1m | _ | ''
tidur 1
bersih
gema
gema
gema
gema
echo $ red "Alat akan di instal dalam waktu 5 detik"
echo $ red "harap tunggu menunggu yaa sayang: v"
tidur 1
gema '' '\ a
\ 033 [36; 1m ____
\ 033 [36; 1m | ___ |
\ 033 [36; 1m | ___ \
\ 033 [36; 1m ___) |
\ 033 [36; 1m | ____ / '' '
tidur 1
bersih
gema
echo $ pur "===================" $ cy "==================="
echo $ i "Ôûê" $ pu "AUTHOR: xroOtZz & IAmSyamsir" $ i "Ôûê"
echo $ i "Ôûê" $ pu "FACEBOOK: IamSyamsir" $ i "Ôûê"
echo $ i "Ôûê" $ pu "GMAIL: xrootz404
@ gmail.com "$ i" Ôûê "
echo $ i "Ôûê" $ pu "TIM: NoName Te4m & Otodidak" $ i "Ôûê"
echo $ pur "===================" $ cy "==================="
gema
echo $ cy "MeNu BRUTEPORCE NEWS:"
echo $ ku "[" $ me "1" $ ku "]" $ i "LAPORAN FB"
echo $ ku "[" $ me "2" $ ku "]" $ i "BRUTE PORCE"
echo $ ku "[" $ me "3" $ ku "]" $ i "BRUTE FORCE FB CRACKER"
echo $ ku "[" $ me "4" $ ku "]" $ i "MBF"
echo $ ku "[" $ me "5" $ ku "]" $ i "CLONING YAHOO"
echo $ ku "[" $ me "6" $ ku "]" $ i "OSIF"
echo $ ku "[" $ me "7" $ ku "]" $ i "PROFILEGUARD FB"
echo $ ku "[" $ me "8" $ ku "]" $ i "BOT REAKSI FACEBOOK"
echo $ ku "[" $ me "9" $ ku "]" $ i "CALL SPAM"
echo $ ku "[" $ me "10" $ ku "]" $ i "SPAM WHATSHAPP"
echo $ ku "[" $ me "11" $ ku "]" $ i "YOUTUBE-DL"
echo $ i "[" $ me "12" $ i "]" $ ku "EXIT"
gema
echo $ pur "Ôò¡ÔöÇ [" $ pur "Masukkan pilihan Anda" $ pur "]"
baca -p "Ôò¦ÔöÇ >>" pil

if [$ pil = 1]
kemudian
bersih
figlet -f miring "SABAR SAIKO SUNDALA" | lolcat
tidur 1
git clone https://github.com/IlayTamvan/Report.git
laporan cd
unzip Report.zip
python2 Report.py
fi

if [$ pil = 2]
kemudian
bersih
figlet -f miring "SABAR SAIKO SUNDALA" | lolcat
tidur 1
git clone https://github.com/edi-nasa12/fb-brute.git
cd fb-brute
python2 brute.py
fi

if [$ pil = 3]
kemudian
bersih
figlet -f miring "SABAR SAIKO SUNDALA" | lolcat
tidur 1
git clone https://github.com/FR13ND8/Fb-Cracker-v.3.git
cd Fb-Cracker-v.3
python2 crack.py
fi

if [$ pil = 4]
kemudian
bersih
figlet -f miring "SABAR SAIKO SUNDALA" | lolcat
tidur 1
git clone https://github.com/edi-nasa12/Windows7.git
cd Windows7
python2 MBF.py
fi

if [$ pil = 5]
kemudian
bersih
figlet -f miring "SABAR SAIKO SUNDALA" | lolcat
tidur 1
git clone https://github.com/edi-nasa12/YahooCloning.git
cd YahooCloning
permintaan instalasi pip2 memekanisasi
persyaratan pemasangan pip2
python2 cloning.py
fi

if [$ pil = 6]
kemudian
bersih
figlet -f miring "SABAR SAIKO SUNDALA" | lolcat
tidur 1
git clone https://github.com/edi-nasa12/OSIF.git
cd OSIF
pip2 instal -r requirement.txt
python2 osif.py
fi

if [$ pil = 7]
kemudian
bersih
figlet -f miring "SABAR SAIKO SUNDALA" | lolcat
tidur 1
git clone https://github.com/edi-nasa12/profileguard.git
cd profileguard
php guard.php
fi

if [$ pil = 8]
kemudian
bersih
figlet -f miring "SABAR SAIKO SUNDALA" | lolcat
tidur 1
git clone https://github.com/edi-nasa12/ROBOT.git
cd ROBOT
python2 blackbot.py
fi

if [$ pil = 9]
kemudian
bersih
figlet -f miring "SABAR SAIKO SUNDALA" | lolcat
tidur 1
git clone https://github.com/Aditya021/SpamCall
cd SpamCall
php SpamCall.php
fi

if [$ pil = 10]
kemudian
bersih
figlet -f miring "SABAR SAIKO SUNDALA" | lolcat
tidur 1
git clone https://github.com/edi-nasa12/LITESPAM.git
cd LITESPAM
sh LITESPAM.sh
fi

if [$ pil = 11]
kemudian
bersih
figlet -f miring "SABAR SAIKO SUNDALA" | lolcat
tidur 1
git clone https://github.com/edi-nasa12/youtube.git
cd youtube
sh edi.sh
sh polder.sh
fi

if [$ pil = 12]
kemudian
bersih
figlet -f miring "SABAR SAIKO SUNDALA" | Ulolcat
tidur 2
echo $ cy "Terima Kasih sudah mengunakan Alat ini"
tidur 2
echo $ i "Gunakanlah Dgn bijak, semoga bermanfaat Buat kamu"
tidur 2
echo $ pur "Jika Ada Kesalahan Anda Bisa Bertanya & Anda Berpikir Sendiri"
tidur 2
echo $ ku "Facebook:" $ i "IamSyamsir"
echo $ ku "Gmail:" $ i "xrootz404@gmail.com & iamsamsir404@gmail.com"
echo $ ku "WhatsApp:" $ i "0887882xxxx"
tidur 2
echo $ pur "Terima kasih Yang Sudah Mendukung dengan Tools ini"
tidur 2
echo $ pur "Alat Ini Dibuat Oleh xroOtZz Dan Di Edit Oleh Iamsyamsir"
keluar
fi